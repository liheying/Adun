...
int main(int argc, char *argv[])
{
	...
	remote_mprotect(pid, mem_addr, 4096, PROT_EXEC | PROT_READ);
	...
	if (0)
	{
		do
		{
			ptrace(PTRACE_SYSCALL, ret_pid, NULL, 0);
			int status;
			int ret = waitpid(-1, &status, WSTOPPED | __WALL);
			...
			{
				struct user_regs_struct regs;
				ptrace(PTRACE_GETREGS, ret_pid, 0, &regs);
				printf("rax = %lld\n", regs.orig_rax);
				int val = ptrace(PTRACE_PEEKTEXT, ret_pid, (void *)regs.rip, 0);
				if (regs.orig_rax == -1)
				{
					printf("failed [rip]: %x rax=%lld, rbx=%lld, rip=%lld, r12=%lld\n", val, regs.rax, regs.rbx, regs.rip, regs.r12);
				}
				else
				{
					printf("succes [rip]: %x\n", val);
